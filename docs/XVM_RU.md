...
```
Использование: Gibbed.MadMax.XvmDecompile.exe [ОПЦИИ] input_xvmc_или_директория [output.xvm]
...
**Пакетный режим:** Если вместо файла указана директория, все `.xvmc` файлы декомпилируются рекурсивно. Можно перетащить папку на `.exe` в проводнике Windows.
...
### Соглашение о сохранении по индексу

```
push value         # сохраняемое значение
push obj           # целевой объект (список/массив)
push index         # индекс в объекте
