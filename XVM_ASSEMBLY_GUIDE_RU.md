...
| `name_hash` | `0xHEX` | Хеш имени модуля (**опционально** — вычисляется из `name`) |
...
> **Совет:** `name_hash` опционален — если не указан, ассемблер вычисляет его
> автоматически из имени модуля через Jenkins-хеш. При создании нового модуля
> с нуля достаточно указать `name` и `source_hash`.
...
| Поле | Описание |
|------|----------|
| `hash` | Хеш имени функции (**опционально** — вычисляется из имени) |
| `args` | Количество аргументов функции |
| `locals` | Общее количество локальных переменных (**вычисляется автоматически** — значение в .dis игнорируется) |
| `max_stack` | Максимальная глубина стека (**вычисляется автоматически** — значение в .dis игнорируется) |
...
### Автоматические hash, locals и max_stack

**hash** — если не указан, ассемблер автоматически вычисляет Jenkins-хеш из имени
функции. Например, `HashJenkins("PreInit")` = `0xAD92A91E`. Это позволяет писать
новые функции без ручного вычисления хешей.

**locals** — всегда вычисляется автоматически. Ассемблер сканирует все инструкции
`ldloc`/`stloc`, находит максимальный использованный индекс и устанавливает
`locals = max(максИндекс + 1, args)`. Значение `locals` в `.dis` файле игнорируется.

**max_stack** — всегда вычисляется автоматически методом прямого анализа потока
данных. Ассемблер проходит все пути выполнения (ветвления, циклы) и определяет
точную пиковую глубину стека. Значение `max_stack` в `.dis` файле игнорируется.

Таким образом, минимальные метаданные функции — только количество аргументов:
...
### Отладочные аннотации (@строка:столбец)

Инструкции могут содержать аннотацию `@строка:столбец` в конце строки (после комментариев):
...
Эти аннотации сохраняют номер строки и столбца из исходного скрипта и являются
частью ADF-экземпляра `debug_info`. Дизассемблер выводит их автоматически, когда
`.xvmc` содержит `debug_info`. Ассемблер считывает их обратно для восстановления `debug_info`.

- Формат: `@СТРОКА:СТОЛБЕЦ`, где СТРОКА и СТОЛБЕЦ — десятичные числа (uint16)
- Позиция: в самом конце строки, после комментария
- Необязательны: если опущены или все нулевые, экземпляр `debug_info` не генерируется
...
| `stitem` | — | val, obj, idx → | Установить элемент по индексу (push значение, объект, индекс) |
...
> **Примечание:** `hash`, `locals` и `max_stack` вычисляются автоматически —
> обновлять их вручную не нужно.
...
### debug_info полностью восстанавливается

Ассемблер полностью поддерживает round-trip `debug_info`. Когда дизассемблер
обнаруживает экземпляр `debug_info` в ADF-контейнере `.xvmc`, он выводит аннотации
`@строка:столбец` в конце каждой строки инструкции. Ассемблер считывает эти аннотации
и восстанавливает ADF-экземпляр `debug_info` (тип `0xDCB06466`) с данными
строк и столбцов для каждой инструкции.

Если вы пишете скрипт с нуля, аннотации `@строка:столбец` можно не указывать —
ассемблер просто не сгенерирует экземпляр `debug_info`. Игра всё равно будет
выполнять скрипт; `debug_info` используется только для отладочных сообщений
об ошибках с указанием строки исходного кода.
...
### 3. ~~Неверный `max_stack`~~ (Больше не проблема)

`max_stack` теперь вычисляется автоматически ассемблером. Указывать
или обновлять его вручную не нужно.

### 4. ~~Неверный `locals`~~ (Больше не проблема)

`locals` теперь вычисляется автоматически по всем инструкциям `ldloc`/`stloc`.
