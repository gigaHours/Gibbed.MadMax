...
- Дизассемблировать скриптовый байткод XVM (с отладочной информацией)
- Ассемблировать `.dis` файлы обратно в `.xvmc` байткод (включая debug_info)
...
Внутри ADF хранятся до трёх экземпляров:
- `"module"` — основной модуль XVM (тип `0x41D02347`)
- `"debug_info"` (необязательно, тип `0xDCB06466`) — отладочная информация (маппинг инструкций на строки/столбцы исходного кода)
- `"debug_strings"` (необязательно, тип `0xFEF3B589`) — отладочные строки (имена атрибутов и глобалов)

**debug_info** содержит для каждой функции массивы номеров строк и столбцов (по одному uint16 на инструкцию), что позволяет игре выводить информативные сообщения об ошибках с указанием позиции в исходном файле.

**Формат debug_info:**

| Смещение | Размер | Поле |
|----------|--------|------|
| 0x00 | 8 | functionsOffset (всегда 0x10) |
| 0x08 | 8 | functionCount |
| 0x10 | N×40 | Записи функций |
| ... | var | Массивы данных (uint16[]) |

**Запись функции (40 байт):**

| Смещение | Размер | Поле |
|----------|--------|------|
| 0x00 | 8 | linenoOffset — смещение массива номеров строк |
| 0x08 | 8 | linenoCount — количество инструкций |
| 0x10 | 8 | colnoOffset — смещение массива номеров столбцов |
| 0x18 | 8 | colnoCount — количество инструкций |
| 0x20 | 4 | nameHash — хеш имени функции |
| 0x24 | 4 | padding (нули) |

Массивы lineno и colno выровнены на 16 байт. Функции сопоставляются с модулем по nameHash.

Подробнее о формате модуля — в разделе [Система скриптов XVM](#система-скриптов-xvm).
...
- Выводит аннотации `@строка:столбец` из `debug_info` (если присутствует в `.xvmc`)

### Gibbed.MadMax.XvmAssemble

Ассемблирование текстовых `.dis` файлов обратно в бинарные `.xvmc`.

```
Gibbed.MadMax.XvmAssemble [OPTIONS]+ input_dis [output_xvmc]
...
**Возможности:**
- Парсит `.dis` формат (вывод дизассемблера)
- Строит таблицы констант, строковый буфер, хеши строк
- Резолвит метки в адреса инструкций
- Кодирует инструкции (16-бит: операнд << 5 | опкод)
- Упаковывает модуль в ADF-контейнер
- Восстанавливает `debug_strings` (отладочные имена атрибутов/глобалов)
- Восстанавливает `debug_info` (маппинг инструкций на строки/столбцы) из аннотаций `@строка:столбец`

**Round-trip:**

```
original.xvmc → XvmDisassemble → .dis → XvmAssemble → rebuilt.xvmc
```

Пересобранный `.xvmc` функционально идентичен оригиналу. Различия в бинарном представлении (порядок констант, layout строкового буфера) не влияют на работу скрипта.
